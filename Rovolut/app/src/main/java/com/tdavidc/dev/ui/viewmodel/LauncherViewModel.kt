package com.tdavidc.dev.ui.viewmodel
...
class LauncherViewModel @Inject constructor(authRepository: AuthRepository) : ViewModel() {
    ...
    val combinedLiveData: MediatorLiveData<Boolean> = MediatorLiveData()

    init {
        combinedLiveData.addSource(continueAfterSplashAnimation) { value1 ->
            val value2 = doesHaveSession.value
            if (value1 != null && value2 != null) {
                combinedLiveData.value = value2!!
            }
        }

        combinedLiveData.addSource(doesHaveSession) { value2 ->
